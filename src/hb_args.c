/*
 * ARGS   Provides functionality to parse standard argc/argv in an easy manner.
 *
 * Version:                                     @(#)args.c    0.0.1    09/07/14
...
int args_create_context( args_context_t* ctx, int argc, const char** argv, const args_option_t* opts )
{
    ...
    /* count opts */
    ctx->num_opts = 0;
    args_option_t cmp_opt;
    memset( &cmp_opt, 0x0, sizeof(args_option_t) );
    const args_option_t* opt = opts;
    while( memcmp( opt, &cmp_opt, sizeof(args_option_t) ) != 0 ) {
        if( opt->value == '!' ||
            opt->value == '?' ||
            opt->value == '+' ||
            opt->value ==  0  ||
...
int args_next( args_context_t* ctx )
{
    ...
    const args_option_t* found_opt = 0x0;
    ...
    if( curr_token[1] != '\0' && curr_token[1] != '-' && curr_token[2] == '\0' ) {
        ...
        for( ; i < ctx->num_opts; i++ ) {
            const args_option_t* opt = ctx->opts + i;
            ...
            if( opt->name_short == curr_token[1] ) {
                ...
                if( ( ( ctx->current_index != ctx->argc) && ( ctx->argv[ctx->current_index][0] != '-' ) ) &&
                    ( opt->type == ARGS_OPTION_TYPE_OPTIONAL || opt->type == ARGS_OPTION_TYPE_REQUIRED ) ) {
    ...
    if(found_arg != 0x0) {
        ...
        case ARGS_OPTION_TYPE_FLAG_SET:
        case ARGS_OPTION_TYPE_FLAG_AND:
        case ARGS_OPTION_TYPE_FLAG_OR:
        case ARGS_OPTION_TYPE_NO_ARG:
            /* these types should have no argument, usage error! */
            ctx->current_opt_arg = found_opt->name;
            return '!';

        case ARGS_OPTION_TYPE_OPTIONAL:
        case ARGS_OPTION_TYPE_REQUIRED:
    ...
    /* no argument found */
        ...
        case ARGS_OPTION_TYPE_FLAG_OR:
            *found_opt->flag |= found_opt->value;
            return 0; /* zero is "a flag was set!" */
...
const char* args_create_help_string( args_context_t* ctx, char* buffer, size_t buffer_size )
{
    ...
    for( ; opt_index < ctx->num_opts; ++opt_index ) {
        const args_option_t* opt = ctx->opts + opt_index;
